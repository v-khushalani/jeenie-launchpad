-- Fix streak not incrementing on daily goal completion
-- Problem: last_activity_date was updated for every question, causing "already_updated_today" false positive
-- Solution: Use last_streak_date instead (only set when streak actually increments)
...
begin
  ...
  if v_profile.last_streak_date = v_today::date then
    -- Already had streak increment today; if streak is 0, recover to 1
